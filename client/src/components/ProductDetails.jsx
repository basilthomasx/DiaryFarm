...
//               className="w-full flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-600 text-white py-4 px-6 rounded-lg transition-colors"
//             >
...
//             {/* Add Review Form */}
//             <form onSubmit={handleSubmitReview} className="space-y-4 mb-6 bg-gray-50 p-4 rounded-lg">
//               <div>
...
const ProductDetails = () => {
  // ... keep all existing state and hooks the same ...
  const { id } = useParams();
  const navigate = useNavigate();
  const [product, setProduct] = useState(null);
  const [reviews, setReviews] = useState([]);
  ...
  // ... keep all existing useEffect and handlers the same ...
  useEffect(() => {
    const fetchProductAndReviews = async () => {
      try {
        const [productRes, reviewsRes] = await Promise.all([
